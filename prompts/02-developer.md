...
サブエージェントの並列実行は「別ファイルを触る独立タスク」に限り使ってよい
(例: ロジックファイルと辞書データを分担)。**同一ファイルを複数エージェントで編集するのは禁止**。
UIとロジックのように型でつながる場合は、先に親が型(ResultType)を確定してから分担すること。
迷ったら並列化せず1本道で書く(25分のMVPでは調整コストの方が高くつくことが多い)。
